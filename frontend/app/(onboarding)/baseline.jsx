...
export default function BaselineScreen() {
  ...
  const [scores, setScores] = useState({}); // { subject_id: { correct, total } }
  ...
  useEffect(() => {
    async function fetchBaseline() {
      ...
      // Grab 3 PYQs per selected subject
  ...
  if (questions.length === 0 || current >= questions.length) {
    // No questions or done — go to home
    finish();
    return null;
  }
  ...
  function handleAnswer(answer) {
    ...
    setRevealed(true);
    // For now we just count attempts; in a real build the pyqs table
    // would have an `answer` field to compare against.
    setScores((prev) => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-slate-900">
      ...
      <View className="flex-1 px-6 pt-8">
        ...
        {/* Since PYQs are open-ended, we ask the student to self-assess */}
        {!revealed && (
